...
class InfoModel(mesa.Model):
    ...
    def __init__(
        ...
        self.datacollector = mesa.datacollection.DataCollector(
            {
                ...
                "Stressed": lambda m: self.count_type(m, "Stressed"),
        ...
        for (x,y) in agent_locations:
            agent = InfoAgent((x, y), self, self.personality, self.density)
  
            if self.all_corners: # If you want all the corners to start.
                if (x,y) in [(0,0), (0,24), (24,0), (24,24)]:
                    agent.condition = "Informed"
                    agent.action_queue = [agent.try_disseminate]
           
            elif self.initial_outbreak != 1:
                pass
        ...
        if self.initial_outbreak != 1:
            for agent in self.sample_agents(agent_locations, self.initial_outbreak):
                agent.condition = "Informed"
                agent.action_queue = [agent.try_disseminate]
